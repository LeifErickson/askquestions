...
class imglib extends db_connect
{
    ...
    public function createAnswerImg($imgFilename)
    {
        ...
        if ($this->checkImg($imgFilename)) {
            ...
            if ($type == IMAGETYPE_JPEG || $type == IMAGETYPE_PNG) {
                ...
                $this->img_resize($imgFilename, TEMP_PATH.$imgOrigin, 500, 0);
        ...
        if ($result['error'] === false) {
            ...
            $response = $cdn->uploadAnswerImg(TEMP_PATH.$imgOrigin);
    ...
    public function createCover($imgFilename)
    {
        $result = array("error" => true);
        ...
        if ($result['error'] === false) {
            ...
            $response = $cdn->uploadCover(TEMP_PATH . $imgOrigin);
    ...
    public function createPhoto($imgFilename)
    {
        ...
        if ($this->checkImg($imgFilename)) {
            ...
            if ($type == IMAGETYPE_JPEG) {
                ...
                $photo = new photo($this->db, $imgFilename, 100);
                imagejpeg($photo->getImgData(), TEMP_PATH.$imgThumbBig, 100);
                unset($photo);

                $photo = new photo($this->db, $imgFilename, 65);
