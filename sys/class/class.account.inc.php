...
class account extends db_connect
{
    ...
    public function signup($username, $fullname, $password, $email, $language = '')
    {
        ...
        $accountState = ACCOUNT_STATE_DEACTIVATED;

        $stmt = $this->db->prepare("INSERT INTO users (state, login, fullname, passw, email, salt, regtime) value (:state, :username, :fullname, :password, :email, :salt, :createAt)");
        ...
        if ($stmt->execute()) {
            ...
            $this->setLanguage(helper::getLang($language));
    ...
    public function get()
    {
        ...
        if ($stmt->execute()) {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $questions->setRequestFrom($this->getId());
